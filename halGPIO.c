...
// In your C file (api.c or main.c)
#pragma DATA_SECTION(fileManager, ".filemanager")
#pragma location=FILEMANAGER_ADDR

FileManager fileManager = {
    .pointers = {},   // all pointers = NULL
    .FileNames = {{0}},   // all strings empty
    .FileSizes = {0},     // all sizes = 0
    .types = {0}          // all types = '\0'
};
int fileManger_numFiles = 0 ;
...
void initFileManagerInFlash() {
    FlashConfig();           // configure flash
    init_flash_write(FILEMANAGER_ADDR);  // start flash write at calculated address
    int i;
    for(i = 0; i < sizeof(FileManager); i++) {
        write_flash_char(NULL,1);
    }

    disable_flash_write();
}
void pwmOutServoConfig(int freq){
      Timer_1_CCR0 = (int) 21000;
      Timer_1_CCR2 = (int) freq;                 // PWM Duty Cycle
                                                 // Min=385,
                                                 //Max = 2005 (180 deg)
      Timer_1_CCTL2 = OUTMOD_7;                    //pwm mode

      Timer_1_CTL =  MC_1 +Timer_1_SSEL2 ;       //upmode up CCR0 + SMCLK
}
...
void pwmOutTrigConfig(){
      ...
      Timer_2_CTL =  MC_1 +Timer_2_SSEL2 ;       //upmode up CCR0 + SMCLK
}
...
void init_flash_write_manger(int addr){
    Flash_ptr = (char *) addr;                // Initialize Flash pointer
    FCTL1 = FWKEY + WRT;                      // Set WRT bit for write operation
    FCTL3 = FWKEY;                            // Clear Lock bit
}
...
void write_with_addr_flash_int(int value, int addr){
    ...
    FCTL3 = FWKEY;                            // Clear Lock bit
    FCTL1 = FWKEY + WRT;                      // Set WRT bit for write operation
    *Flash_ptr++ = value & 0xFF;        // LSB
    *Flash_ptr   = (value >> 8) & 0xFF; // MSB
}

void write_flash_char(char value, int write_flag_up) {
    if (write_flag_up) {
        // Write up (increment after writing for FileManger)
        *Flash_ptr++ = (char)value;
    } else {
        // Write down (decrement after writing for scripts)
        *Flash_ptr-- = (char)value;
    }
}
void write_flash_int(int value, int write_flag_up) {
    if (write_flag_up) {
        // Write up (increment after writing for FileManger)
        *Flash_ptr++ = value;
    } else {
...
//---------------------------------------------------------------------
	    if(PBsArrIntPend & PB0) {
          ...
          if (state == state5 && fileManger_numFiles !=0 && first_pb1 ==0 ){
              if (scroll_idx >= fileManger_numFiles) {
                  scroll_idx = 0;
                  }
              lcd_clear();
              lcd_puts("Choose File:");
              lcd_new_line ;
              char* name = fileManager.FileNames[scroll_idx];
              lcd_puts(name);
              scroll_idx ++;
	          ...
	          PBsArrIntPend &= ~PB1;
	          __bic_SR_register_on_exit(LPM0_bits); //out from sleep
	          if (state == state5 && first_pb1 == 0){
	              state_script = 10 + scroll_idx;
	              //send_char('a');
	              first_pb1 = 1 ;
	              lcd_clear();
	              }
	              ...
	              first_pb1 = 0 ;
	              PBsArrIntPend |= PB0; // go back to show menu
...
#endif
{
 ...
 //============== Receive scripts ===============
        ...
        static unsigned char len_high = 0;
        if (script_index == 0) {
            // First byte = high part (could be 0 if length < 256)
            len_high = UCA0RXBUF;
            script_index = 1;
        }
        else if (script_index == 1) {
            // Second byte = low part
            unsigned char len_low = UCA0RXBUF;

            script_length = ((unsigned int)len_high << 8) | len_low;
            script_index = 2;
            scriptFlag = 2;  // ready to receive script data
        }

    }
     else if (scriptFlag == 2){

         int addr_point = (int)&fileManagerPtr->pointers[idx_script];

         int offset = fileManager.pointers[idx_script - 1] + script_length;
         write_with_addr_flash_int(offset,addr_point);
         script_index = 0;

        scriptFlag = 3;
        int addr = (int)&fileManagerPtr->types[idx_script-1] ;
        write_with_addr_flash_char(UCA0RXBUF, addr);
        }
     else if (scriptFlag == 3){
         char c = UCA0RXBUF;

             if (script_index == 0) {
                 // first byte  initialize flash write
                 FlashConfig();
                 int addr = FILE_START_ADDR - fileManager.pointers[idx_script - 1] ;
                 init_flash_write(addr);
             }

             if (script_index < script_length) {
                 write_flash_char(c, 0);  // write straight to flash
                 script_index++;
             }

             if (script_index >= script_length) {
                 disable_flash_write();  // finish flash
                 scriptFlag = 0;
                 script_index = 0;
