...
//                __delay_cycles(200000);
                ...
                flash_write_calib(sample1, 0X1080 + calibrate_index);
                flash_write_calib(sample2, 0X1080 + calibrate_index + 20);
                calibrate_index++;
...
void script_fsm(){
 ...
 while(state == state5){
    ...
    case upload_file1:
        ...
        addFile("script0", script_length); // NEED TO ADD ARRAY OF POINTERS TO THE START ADDRRESS OF EVERY FILE
        state_script = sleep;
    ...
    case upload_file10:
        ...
        enable_interrupts();
        enterLPM(lpm_mode);  // expect to receive script data
        send_char('10');  // Send acknowledge to the PC side after receiving the script successfully.
        addFile("script9", script_length);
        state_script = sleep;
...
void move_servo(int angle){
    angle = (angle * 9) + 385;     // angle*(2000-390)/180 + 100
...
void servo_scan(int angle1, int angle2, int dist_or_light_flag){
    ...
    TIMER1config();
    TIMER2config();
    curr_angle = (angle1 * 9) + 385;     // angle*(2005-385)/180 + 385     // innitialize
    width =      (angle2 * 9) + 385 ;     // angle*(2005-385)/180 + 385     // up to 180 deg
    ...
    while(curr_angle <= width){
        enable_timerA0(); //enable the timer with delay of d[msec] for pc side to update
        enterLPM(lpm_mode);
        disable_timerA0();
        curr_angle += 9;
        pwmOutServoConfig(curr_angle);
        __delay_cycles(50000); // delay for servo to stabilize
    ...
    pwmOutServoConfig(1195); // INIT to 90 deg
    __delay_cycles(1500000);
...
//------------------------------------------------------------------------------
// Measure distance [0-400 cm] using ultrasonic sensor
//------------------------------------------------------------------------------
...
void meas_and_send_distance(){
    ...
    distance = measure_distance(); // measure distance
                                    // using ultrasonic sensor [0-400 cm]
...
//    else if (LDR_index == 2){ // if index is 2 , we can calculate avg
//        sample2 = sample;
...
//                              Scripts
//------------------------------------------------------------------------------
void read_txt(int script_num){
    char line_one[16] ;
    char line_two[16] ;
    char * read_ptr = FILE_START_ADDR -  fileManager.pointers[script_num - 1];
    int i = 0 ;
    int total_chars_read = 0;

    while (total_chars_read != fileManager.FileSizes[script_num - 1]){
            for (i = 0 ; i <32 && total_chars_read < fileManager.FileSizes[script_num - 1]; i++ ){
                if (i < 16) {
                      // First 16 characters go to line_one
                      line_one[i] = *read_ptr;
                }
                else {
                      // Next 16 characters go to line_two
                      line_two[i - 16] = *read_ptr;
                            }
                 read_ptr--; // Move to next character (decreasing addresses)
                 total_chars_read++;
...
void play_script(int script_num){
    ...
    if (fileManager.types[script_num - 1] == 's'){
         ...
         // after receiving a whole line:
            ...
            switch(opcode_int){
                ...
                case 7:
                    ...
                    lcd_cursor2();
                    lcd_puts("Environment");
                    calc_and_send_angle(arg1_int);
                    __delay_cycles(50000);
                    calc_and_send_angle(arg2_int);
...
void addFile(const char * filename, int fileSize) {
    int addr ;
    if (fileManger_numFiles >= MAX_FILES) {
        // Maximum number of files reached
        return;
    }

    int i = fileManger_numFiles ;
    int j =0;

    // --- Write FileNames[i] ---
    for (j = 0; j < MAX_FILENAME_LEN - 1 && filename[j] != '\0'; j++) {
        addr = (int)&fileManagerPtr->FileNames[i][j];
        write_with_addr_flash_char(filename[j], addr);
    }
    // Write null terminator
    addr = (int)&fileManagerPtr->FileNames[i][MAX_FILENAME_LEN - 1];
    write_with_addr_flash_char('\0', addr);

    // Store file size
    addr = (int)&fileManagerPtr->FileSizes[i];
    write_with_addr_flash_int(fileSize,addr);
...
//------------------------------------------------------------------------------
// Write script to flash memory
//------------------------------------------------------------------------------
void flash_write(int script_num ,int write_flag_up){

    if(script_num > fileManger_numFiles){
        // Invalid script number
        return;
    }

    int i = 0;
     // get pointer from FileManager
    int max_size = fileManager.FileSizes[script_num - 1];  // max bytes to write

    FlashConfig();           // configure flash
    int addr = FILE_START_ADDR - fileManager.pointers[script_num - 1];
    init_flash_write(FILE_START_ADDR - fileManager.pointers[script_num - 1]);  // start flash write at calculated address

    //char *script_string = fileManager.FileNames[script_num]; // or pointer to data
    while(i < max_size && script_string[i] != '$'){  // write until '$' or end
        write_flash_char(script_string[i],write_flag_up);
        i++;
    }

    disable_flash_write();   // finish flash write
}
