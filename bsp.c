...
//    PBsArrIntPend         &= ~PB0;         // clear pending interrupt
    ...
    PBsArrPortSel         &= ~0x81;         // PB0 is I/O
    PBsArrPortDir         &= ~0x81;         // PB0 is Input
    PBsArrIntEdgeSel      |=  0x81;         // PB0 is pull-up mode
    PBsArrIntEn           |=  0x81;         // enable PB0 interrupt
    PBsArrIntPend         &= ~0xFF;         // clear pending interrupt
...
void TIMER3config(void){    /* third Timer config for ultrasonic sensor */
    ...
    Timer_2_CTL =  MC_1 +Timer_2_SSEL2 ;                // upmode CCR0 + SMCLK
